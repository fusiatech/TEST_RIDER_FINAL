...
import { createLinearConnectUrl, finalizeLinearCallback } from '@/server/integrations/linear-service'
...
function resolveCallbackUrl(request: NextRequest, callbackUrl?: string): string {
  const fallback = `${request.nextUrl.origin}/api/integrations/linear/callback`
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!parsed.success) {
    return badRequest(`Invalid Linear connect payload: ${parsed.error.message}`)
  }
  ...
  if (parsed.data.code && parsed.data.state) {
    const result = await finalizeLinearCallback({
      state: parsed.data.state,
      code: parsed.data.code,
      callbackUrl,
    })

    await auditIntegrationSecretRotate('linear', { userId: authResult.user.id })
    await auditIntegrationConnect('linear', { userId: authResult.user.id, mode: 'oauth' })
    return NextResponse.json(result)
  }

  const connect = await createLinearConnectUrl(authResult.user.id, callbackUrl)
  await auditIntegrationConnect('linear', { userId: authResult.user.id, requested: true })
