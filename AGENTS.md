...
- **Prompt escaping:** `server/cli-runner.ts` writes prompts to `/tmp/swarm-prompt-*.txt` temp files instead of passing inline to bash. This avoids shell-escaping issues with backticks, dollar signs, and newlines in LLM prompts. Temp files are cleaned up on process exit/kill.
- **CLI availability:** `server/orchestrator.ts` calls `detectInstalledCLIs()` at pipeline start and filters `enabledCLIs` to only installed CLIs. If none are available, a mock agent at `/tmp/mock-agent.sh` is used as fallback.
...
- **Structured logger:** `server/logger.ts` — creates component-scoped loggers that write color-coded console output AND JSON lines to `/tmp/swarm-ui.log`. Used in `ws-server.ts` and `server.ts`.
